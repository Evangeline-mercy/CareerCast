...
try:
    ...
    print("Verify with: python -c \"import xgboost; print(xgboost.__version__)\"")
    sys.exit(1)
...
OUTPUT_DIR = os.path.join("results", "milestone2_xgboost")
...
def main():
    print("=" * 70)
    print("CAREERCAST MILESTONE 2 - XGBOOST (profile_training track)")
    print("=" * 70)
    ...
    try:
        if not os.path.isfile(INPUT_PATH):
            fail(f"Dataset not found at '{INPUT_PATH}'.")
        ...
        fail(f"Failed to load dataset: {e}", e)

    n_careers = df["career"].nunique()
    log("Dataset loaded.")
    print(f"Dataset size: {len(df)} rows")
    print(f"Number of career classes: {n_careers}")

    X_text = df["skills"].astype(str)
    y_raw = df["career"].astype(str)
    ...
    try:
        ...
        fail(f"Train/test split failed (check class balance): {e}", e)
    ...
    try:
        ...
        fail(f"TF-IDF vectorization failed: {e}", e)
    ...
    log(f"Reducing dimensionality with TruncatedSVD ({SVD_COMPONENTS} components)...")
    try:
        ...
        fail(f"TruncatedSVD failed: {e}", e)

    log("SVD complete.")
    print(f"SVD dimensions: {X_train_svd.shape[1]}")
    print(f"SVD explained variance (sum): {explained_variance:.4f}")
    ...
    log(f"Setting up GridSearchCV ({CV_FOLDS}-fold, {total_fits} total fits)...")
    try:
        ...
        fail(f"Failed to set up GridSearchCV: {e}", e)
    ...
    try:
        ...
        fail(f"Hyperparameter search failed: {e}", e)

    best_params = grid_search.best_params_
    cv_accuracy = grid_search.best_score_
    log(f"Hyperparameter search complete. ({search_time:.1f}s)")
    print(f"Best XGBoost parameters: {best_params}")
    print(f"Cross-validation accuracy: {cv_accuracy:.4f}")

    log("Training final XGBoost model on full training set...")
    try:
        ...
        fail(f"Final model training failed: {e}", e)
    log(f"Training completed in {training_time:.1f} seconds.")
    ...
    try:
        ...
        true_career_labels = label_encoder.inverse_transform(y_test)
        predicted_career_labels = label_encoder.inverse_transform(y_pred)

        predictions_df = pd.DataFrame({
            "skills": X_test_text.reset_index(drop=True),
            "true_career": true_career_labels,
            "predicted_career": predicted_career_labels,
        })
    ...
    if missing_after_save:
        fail(f"The following expected output files were not found after saving: {missing_after_save}")
